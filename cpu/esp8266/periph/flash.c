...
static esp_spi_flash_chip_t* _flashchip = NULL;
...
void flash_drive_init (void)
{
    ...
    if ((_flashchip->deviceId & 0xff) >= 0x12 &&
        (_flashchip->deviceId & 0xff) <= 0x18) {
        _flashchip->chip_size = flash_sizes[(_flashchip->deviceId & 0xff) - 0x12] << 10;
    }
    else {
        LOG_TAG_ERROR("spi_flash", "could not determine flash size, "
